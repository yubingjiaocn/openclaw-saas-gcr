...
kubectl create secret generic platform-api-config \
  ...
  --from-literal=SQS_QUEUE_URL="https://sqs.<region>.amazonaws.com/<account>/openclaw-saas-usage-events" \
  --from-literal=USAGE_EVENTS_QUEUE_URL="<same as SQS_QUEUE_URL>" \
  --from-literal=ECR_REGISTRY="<account>.dkr.ecr.<region>.amazonaws.com" \
  ...
  --from-literal=AWS_PARTITION="aws" \
  --from-literal=AWS_ACCOUNT_ID="<account-id>" \
  --from-literal=AVAILABLE_CHANNELS="" \
  ...
  --from-literal=METRICS_EXPORTER_TAG="v0.3.0" \
  --dry-run=client -o yaml | kubectl apply -f -
```

See `infra/.env.example` for variable descriptions.
